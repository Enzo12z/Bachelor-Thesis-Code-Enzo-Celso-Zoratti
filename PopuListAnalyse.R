...
popuList <- read.csv("/Users/enzo/Desktop/BA Datenanalyse/The PopuList 3.0 15.51.19.csv", 
                     sep = ";", stringsAsFactors = FALSE)

farright <- popuList %>%
  filter(farright == 1, !is.na(parlgov_id), parlgov_id != "") %>%
  pull(parlgov_id) %>%
  as.integer()

rw_voteshare2020_popuList <- election_eu %>%
  filter(election_type == "parliament",
         !is.na(vote_share),
         party_id %in% farright,
         as.Date(election_date) <= as.Date("2020-12-31")) %>%
  group_by(country_name, election_date) %>%
  summarise(rw_vote = sum(vote_share, na.rm = TRUE)) %>%
  group_by(country_name) %>%
  slice_max(election_date, n = 1) %>%                
  ungroup()   

rw_voteshare2020_popuList <- rw_voteshare2020_popuList %>%
  bind_rows(data.frame(country_name = "Iceland", rw_vote = 0, 
                       election_date = NA_character_))


rw_voteshare2010_popuList <- election_eu %>%
  filter(election_type == "parliament",
         !is.na(vote_share),
         party_id %in% farright,
         as.Date(election_date) <= as.Date("2010-12-31")) %>%
  group_by(country_name, election_date) %>%
  summarise(rw_vote = sum(vote_share, na.rm = TRUE)) %>%
  group_by(country_name) %>%
  slice_max(as.Date(election_date), n = 1) %>%
  ungroup()  

rw_voteshare2010_popuList <- rw_voteshare2010_popuList %>%
  bind_rows(data.frame(country_name = c("Iceland", "Spain"),
                       rw_vote      = c(0, 0),
                       election_date = NA_character_))
...
issp2020_analysed <- issp2020 %>% left_join(rw_voteshare2020_popuList, by ="country_name") #for robustness check change to rw_voteshare2020 and same beneath
...
issp2010_analysed <- issp2010 %>%left_join(rw_voteshare2010_popuList, by = "country_name")
...
#sens analysis (after imai & yamamoto)

sens.out <- medsens(med.out, effect.type="indirect", #confirms null finding further
                    rho.by=0.1, sims=1000)
summary(sens.out)
