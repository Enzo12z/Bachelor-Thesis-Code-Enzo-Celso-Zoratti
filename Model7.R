...
#>> Right wing vote share : populist: treshhold = 7;  columN nr.11 : left_right 
...
#------#filter election -> right wing parties (right-wing populist parties rwpp) https://stackoverflow.com/questions/45884261 / https://www.statology.org/dplyr-filter-multiple-conditions/
         
rw_7 <- election_eu%>% filter(election_type == "parliament",
                              !is.na(left_right),
                              left_right >= 7)          #fiters  for parliament elections with available left_right data (NO N.A) and parties with >- 7 entryy (here rwPp)


#rw_7 vote share per country 2020&2010

rwpp7_voteshare2010 <- election_eu %>%
...
rwpp7_voteshare2020 <- election_eu %>%
  filter(election_type == "parliament",
         !is.na(left_right),!is.na(vote_share),
...
issp2020_analysed <- issp2020 %>% left_join(rwpp7_voteshare2020, by ="country_name") #for robustness check change to rw_voteshare2020 and same beneath
...
issp2010_analysed <- issp2010 %>%left_join(rwpp7_voteshare2010, by = "country_name")
...
coeftest(med.fit, vcov = vcovCL(med.fit, cluster = ~country_name)) #check after running mediation
...
coeftest(out.fit, vcov = vcovCL(out.fit, cluster = ~country_name)) #check after running mediatn
