#this code contains all variants of the code for rw6 and rw7 // see other seperate files for rw8 & popuList. Same methods used there though.
#if run at once / together, output is valid for rw7 analysis
...
rw_6 <- election_eu %>%filter(election_type == "parliament",
         !is.na(left_right),
         left_right >= 6)           #fiters out for parliament elections with available left_right data (NO N.A) and parties with >- 6 entry (here rw)

rw_7 <- election_eu%>% filter(election_type == "parliament",
         !is.na(left_right),
         left_right >= 7)          #fiters  for parliament elections with available left_right data (NO N.A) and parties with >- 6 entryy (here rwP)


#rw_6 vote share per country 2010& 2020

rw_voteshare2010 <- election_eu %>%
  filter(election_type == "parliament",       #Filters for: parliament election; available left_right and vote_share entries & right-wing (treshold of 6)
         ...
         left_right >= 6,as.Date(election_date) <= as.Date("2010-12-31")) %>%       #and filter for date (till end of 2020; see slice_max beneath that works now)
...
rw_voteshare2020 <- election_eu %>%
  filter(election_type == "parliament",    
         !is.na(left_right),!is.na(vote_share),
         left_right >= 6,as.Date(election_date) <= as.Date("2020-12-31")) %>%  
...
#rw_7 vote share per country 2020&2010 REPEAT SAME THING FROMABOVE 

rwpp_voteshare2010 <- election_eu %>%
  filter(election_type == "parliament",
         !is.na(left_right),!is.na(vote_share),
         left_right >= 7,as.Date(election_date) <= as.Date("2010-12-31")) %>%
  group_by(country_name, election_date) %>%
  summarise(rw_vote = sum(vote_share)) %>%
  group_by(country_name) %>%
  slice_max(election_date, n = 1) %>%
...
rwpp_voteshare2020 <- election_eu %>%
  filter(election_type == "parliament",
         !is.na(left_right),!is.na(vote_share),
         left_right >= 7,as.Date(election_date) <= as.Date("2020-12-31")) %>%
  group_by(country_name, election_date) %>%
  summarise(rw_vote = sum(vote_share)) %>%
  group_by(country_name) %>%
  slice_max(election_date, n = 1) %>%
...
issp2020 <- issp2020 %>%            #issp countries have numbers, ie. iso codes but parlgov uses eihter full or abbrev for countres 
...
issp2020_analysed <- issp2020 %>% left_join(rwpp_voteshare2020, by ="country_name") #for robustness check change to rw_voteshare2020 and same beneath
...
issp2010_analysed <- issp2010 %>%left_join(rwpp_voteshare2010, by = "country_name")
